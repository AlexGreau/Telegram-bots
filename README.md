...
Send `/update` to the bot instead of doing the steps above by hand. It runs
`deploy.sh` - fetch, fast-forward pull, and `pip install` only if
`requirements.txt` changed - replies with the new commit, then exits.
`Restart=always` in the unit brings it straight back on the new code, so no
sudo and no open ports are involved.

Restricted to `UPDATE_ALLOWED_IDS`, which falls back to `ASSIST_ALLOWED_IDS`
when unset. Message `@userinfobot` on Telegram to find your numeric ID.

The pull is `--ff-only`, so it refuses rather than clobbering anything if the
Pi has local commits or edits. If it reports nothing to pull, you are already
